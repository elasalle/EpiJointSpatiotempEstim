...
for data_i, data_index in enumerate(data_indices):
    ...
    all_params_comb = list(product(taus))
    ntau = len(taus)
    all_i_comb = list(product(range(ntau)))

    print("Start parallel computations...")

    ti = time()
    res = Parallel(n_jobs=-1)(
        delayed(je.RL_wAO_noOutput)(data["Z"], parameters["options"], max_iter, 0., 0., 0., omegas, 
                                    init_method, init_param = {"tau": p_set[0], "options": parameters["options"]}, 
                                    updateL=True, verbose=False, folder=res_folder, names=names_from_indices(i_set, data_index)) 
                                    for p_set, i_set in zip(all_params_comb, all_i_comb)
        )
    tf = time()
