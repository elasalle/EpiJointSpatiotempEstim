...
for data_i, data_index in enumerate(data_indices):
    ...
    res = Parallel(n_jobs=-1)(
        delayed(je.RL_wAO_noOutput)(data["Z"], parameters["options"], max_iter, *p_set, omegas, init_method, updateL=False, verbose=False, folder=res_folder, names=names_from_indices(i_set, data_index)) for p_set, i_set in zip(all_params_comb, all_i_comb)
        )
    tf = time()
