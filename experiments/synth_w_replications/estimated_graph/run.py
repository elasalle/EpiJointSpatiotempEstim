...
data_indices = range(19, 20)
max_iter=10
omegas=None
init_method="epiEstim"
lambda_Ts = 1*np.logspace(0,2,8)
lambda_Ss = 1*np.logspace(-2,3,8)
lambda_Fros = np.logspace(-3,2,8)
...
Lref = et.get_Lstar(parameters["cluster_sizes"])
...
for data_i, data_index in enumerate(data_indices):
    ...
    res = Parallel(n_jobs=-1)(
        delayed(je.RL_wAO_noOutput)(data["Z"], parameters["options"], max_iter, *p_set, omegas, init_method, Linit=None, updateL=True, verbose=False, folder=res_folder, names=names_from_indices(i_set, data_index)) for p_set, i_set in zip(all_params_comb, all_i_comb)
        )
    tf = time()
