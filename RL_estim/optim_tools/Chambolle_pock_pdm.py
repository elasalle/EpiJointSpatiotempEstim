# from https://github.com/juliana-du/Covid-R-estim/blob/main/include/optim_tools/Chambolle_pock_pdm.py
# downloaded in 2025-03

# modifications by Etienne Lasalle:
# - modif1, 2025-04: initialize the dual variable y
# - modif2, 2025-04: Modification of the stopping criterion. Ensure that objective function has decreased compared to initilization.
# - modif3, 2025-04: Return the dual variable y.
...
def PD_ChambollePock_primal_BP(data, param, op, prox, objective):
    ...
    if not hasattr(param, 'incr'):
        param.incr = 'R'
    if not hasattr(param, "y0"): #modif1
        param.y0 = None
    ...
    objInit = objective.fidelity(x, data) + objective.regularization(op.direct(x), 1) #modif2
    objCurrent = objInit+1 # modif2

    # Main loop
    i = -1
    while (stopCondition > param.tol and i < param.iter - 1) or objCurrent>objInit: # modif2
        i += 1
